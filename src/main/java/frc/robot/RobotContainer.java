...
public class RobotContainer {
      ...
      public RobotContainer() {
            ...
            Command driveSwerve = swerve.driveCommand(
                        () -> MathUtil.applyDeadband(
                                    (-driverController.getLeftY() * (((driverController.getR2Axis() + 1) / 2) + 3) / 4)
                                                * Math.max(1 - ((driverController.getL2Axis() + 1) / 2), 0.3),
                                    Constants.ControllerDeadband),
                        () -> MathUtil.applyDeadband(
                                    (-driverController.getLeftX() * (((driverController.getR2Axis() + 1) / 2) + 3) / 4)
                                                * Math.max(1 - ((driverController.getL2Axis() + 1) / 2), 0.3),
                                    Constants.ControllerDeadband),
                        () -> MathUtil.applyDeadband((-driverController.getRightX()), Constants.ControllerDeadband),
                        false, true); // Control heading with right joystick

            // Heading control swerve command
            // Command driveSwerve = swerve.driveCommand(
            // () -> MathUtil.applyDeadband((-driverController.getLeftY() *
            // (((driverController.getR2Axis()+ 1)/2) + 3)/4) * Math.max(1 -
            // ((driverController.getL2Axis()+ 1)/2), 0.3), Constants.ControllerDeadband),
            ...
            // Constants.ControllerDeadband)); //Control heading with right joystick
      ...
      private void configureBindings() {
            ...
            driverController.R1().whileTrue(swerve.driveCommand(() -> 0, () -> 0,() -> Constants.FieldPoses.Hub.getY() - swerve.getPose().getY(),() -> Constants.FieldPoses.Hub.getX() - swerve.getPose().getX()));
            // driverController.cross().or(driverController.triangle()).onTrue(getAutonomousCommand());
            // operatorController.povLeft().or(driverController.circle()).whileTrue(new
            // LUTAutoShootCommand(shooterSubsystem, feederSubsystem));
            ...
            operatorController.circle().whileTrue(new StartEndCommand(
                        intakeSubsystem::runRollerOuttake,
                        intakeSubsystem::stopRoller,
                        intakeSubsystem));

            operatorController.square().onTrue(
                        new RunCommand(shooterSubsystem::reverse, shooterSubsystem)).onFalse(
            ...
            operatorController.cross().whileTrue(new LUTShootFromNeutral(shooterSubsystem, feederSubsystem));

            operatorController.povUp().onTrue(new pivotPosCmd(pivotSubsystem, -20, false));
            operatorController.povDown().onTrue(new pivotPosCmd(pivotSubsystem, 90, false));
            ...
            NamedCommands.registerCommand("FullShoot",
                        new ParallelCommandGroup(
                                    swerve.driveCommand(() -> 0, () -> 0,
                                                () -> Constants.FieldPoses.Hub.getY() - swerve.getPose().getY(),
                                                () -> Constants.FieldPoses.Hub.getX() - swerve.getPose().getX()),
                                    new SequentialCommandGroup(new WaitCommand(0.5),
                                                new LUTAutoShootCommand(shooterSubsystem, feederSubsystem))));
